...
import 'package:e_commerce/model/ordersModel.dart';
...
class ProductProvider with ChangeNotifier {
  ...
  List<CartModel> favouriteList=[];
  CartModel checkOutModel;
  CartModel favouriteModel;
      ...
      (element) {
        if (currentUser.uid == element.data()["UserId"]) {
          ordersModel = OrdersModel(
              ...
              productColor: element.data()["ProductColor"],
              productSize: element.data()["ProductSize"],
              productQuetity: element.data()["ProductQuetity"],
              productPrice: element.data()["ProductPrice"]);
          newList.add(ordersModel);
        }
      },
    );
    ordersModelList = newList;
    notifyListeners();
        // print(orders[0].productName);
  }

  List<OrdersModel> get getOrdersModelList {
  ...
  void deleteFavouriteProduct(int index) {
